...
namespace Atomex.Cryptography.BouncyCastle
{
    public class BcEd25519
    {
        ...
        private static readonly object precompLock = new();
        ...
        private static bool DecodePointVar(ReadOnlySpan<byte> p, bool negate, PointExt r)
        {
        ...
        public unsafe static void GeneratePublicKey(ReadOnlySpan<byte> sk, Span<byte> pk)
        {
            ...
            hSpan[..ScalarBytes].CopyTo(sSpan);
            ...
            Span<byte> sig)
        {
            ...
            RSpan.CopyTo(sig);
            SSpan[..ScalarBytes].CopyTo(sig.Slice(PointBytes, ScalarBytes));
        }
            ...
            Span<byte> sig)
        {
            if (!CheckContextVar(ctx, phflag))
                throw new ArgumentException("Invalid context", nameof(ctx));

            using var d = CreateDigest();
            var h = stackalloc byte[GetDigestSize()];
            var hSpan = new Span<byte>(h, GetDigestSize());

            d.Update(sk[..SecretKeySize]);
            d.Finalize(hSpan);
            ...
            ReadOnlySpan<byte> m)
        {
            ...
            var R = sig[..PointBytes]; // from sifOff to sigOff + PointBytes
            ...
            Span<byte> signature)
        {
            ...
            ScalarMultBaseEncoded(extendedPrivateKey[..32], pk);

            using var h = new UnmanagedBytes(GetDigestSize());
            extendedPrivateKey[..GetDigestSize()].CopyTo(h);
            ...
            out SecureBytes publicKey)
        {
            ...
            seed.GetReadOnlySpan()[..SecretKeySize].CopyTo(unmanagedPrivateKey);
