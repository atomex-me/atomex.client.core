...
{
    public class BitcoinBasedKeyTests : KeyTests<BitcoinBasedKey>
    {
        public override int KeySize => 32;
        public override int SignatureSize => 71;

        public override BitcoinBasedKey CreateKey(int keySize, out byte[] seed)
        {
            seed = Rand.SecureRandomBytes(keySize);

            using var secureSeed = new SecureBytes(seed);

            return new BitcoinBasedKey(secureSeed);
        }
    }
}