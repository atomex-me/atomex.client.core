...
{
    public class Bip32TezosExtKeyTests : KeyTests<Bip32TezosExtKey>
    {
        public override int KeySize => 32;

        public override int SignatureSize => 64;

        public override Bip32TezosExtKey CreateKey(int keySize, out byte[] seed)
        {
            seed = Rand.SecureRandomBytes(keySize);

            using var secureSeed = new SecureBytes(seed);

            return new Bip32TezosExtKey(secureSeed);
        }
    }
}