...
namespace Atomex.Client.Core.Tests.Cryptography
{
    public class Sha512Tests
    {
        ...
        public void CanCalculateHash(HashAlgorithm sha512, string data, string hash)
        {
            var hashBytes = sha512.Hash(Encoding.UTF8.GetBytes(data));
        ...
        [Theory]
        [MemberData(nameof(Data))]
        public void CanCalculateHashToSpan(HashAlgorithm sha512, string data, string hash)
        {
            var hashBytes = new byte[HashSize];

            sha512.Hash(Encoding.UTF8.GetBytes(data), hashBytes);
        ...
        [Theory]
        [MemberData(nameof(Data))]
        public void CanVerify(HashAlgorithm sha512, string data, string hash)
        {
            var result = sha512.Verify(
        ...
        [Theory]
        [MemberData(nameof(Data))]
        public void CanCalculateIncrementalHash(HashAlgorithm sha512, string data, string hash)
        {
            var dataBytes = Encoding.UTF8.GetBytes(data);
            var dataBytesSpan = new ReadOnlySpan<byte>(dataBytes);

            using var incrementalHash = sha512.CreateIncrementalHash();
        ...
        [Theory]
        [MemberData(nameof(Data))]
        public void CanCalculateIncrementalHashToSpan(HashAlgorithm sha512, string data, string hash)
        {
            var dataBytes = Encoding.UTF8.GetBytes(data);
            var dataBytesSpan = new ReadOnlySpan<byte>(dataBytes);
        ...
        [Theory]
        [MemberData(nameof(Data))]
        public void CanReinitiateIncrementalHash(HashAlgorithm sha512, string data, string hash)
        {
            using var incrementalHash = sha512.CreateIncrementalHash();
        ...
        [Theory]
        [MemberData(nameof(Data))]
        public void CanReinitiateSeveralTimes(HashAlgorithm sha512, string data, string hash)
        {
            using var incrementalHash = sha512.CreateIncrementalHash();
            incrementalHash.Update(Encoding.UTF8.GetBytes(data));
