...
namespace Atomix.Cryptography.BouncyCastle
{
    public abstract class Ed25519
    {
            ...
            byte[] extendedPrivateKey)
        {
            ...
            byte[] s = new byte[ScalarBytes];
            Buffer.BlockCopy(extendedPrivateKey, 0, s, 0, 32);
            ...
            var d = CreateDigest();
            byte[] h = new byte[d.GetDigestSize()];
            Buffer.BlockCopy(extendedPrivateKey, 0, h, 0, 64);

            try
            {
                ImplSign(d, h, extendedPrivateKey, pk, 0, ctx, phflag, data, 0, data.Length, signature, 0);
