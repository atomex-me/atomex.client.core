...
{
    public class LiteDbAccountDataRepository_OLD : IAccountDataRepository_OLD
    {
        ...
        private const string CurrencyKey           = nameof(WalletAddress_OLD.Currency);
        private const string AddressKey            = nameof(WalletAddress_OLD.Address);
        private const string BalanceKey            = nameof(WalletAddress_OLD.Balance);
        private const string UnconfirmedIncomeKey  = nameof(WalletAddress_OLD.UnconfirmedIncome);
        private const string UnconfirmedOutcomeKey = nameof(WalletAddress_OLD.UnconfirmedOutcome);
        ...
        private const string HasActivityKey        = nameof(WalletAddress_OLD.HasActivity);
        ...
        private const string KeyTypeKey            = nameof(WalletAddress_OLD.KeyType);
        ...
        public LiteDbAccountDataRepository_OLD(
            string pathToDb,
        ...
        public Task<WalletAddress_OLD> GetWalletAddressAsync(
            string currency,
            string address)
        {
            try
            {
                lock (_syncRoot)
                {
                    ...
                    var walletAddress = document != null
                        ? _bsonMapper.ToObject<WalletAddress_OLD>(document)
                        : null;
            ...
            return Task.FromResult<WalletAddress_OLD>(null);
        }

        public Task<WalletAddress_OLD> GetLastActiveWalletAddressAsync(
            string currency,
            uint chain,
            int keyType)
        {
        ...
        public Task<WalletAddress_OLD> GetLastActiveWalletAddressByAccountAsync(
            string currency,
            int keyType)
        {
            try
            ...
            bool includeUnconfirmed = true)
        {
            ...
            try
            {
                lock (_syncRoot)
                {
                    ...
                    var unspentAddresses = addresses
                        .Find(query)
                        .Select(d => _bsonMapper.ToObject<WalletAddress_OLD>(d))
                        .ToList();

                    return Task.FromResult<IEnumerable<WalletAddress_OLD>>(unspentAddresses);
                }
            }
            catch (Exception e)
            {
            ...
            return Task.FromResult(Enumerable.Empty<WalletAddress_OLD>());
        }
        ...
        public Task<WalletAddress_OLD> GetTezosTokenAddressAsync(
            string currency,
        ...
        public Task<IEnumerable<WalletAddress_OLD>> GetTezosTokenAddressesAsync()
        {
            try
            {
                lock (_syncRoot)
                {
                    ...
                    return Task.FromResult<IEnumerable<WalletAddress_OLD>>(addresses);
                }
            }
            catch (Exception e)
            {
        ...
        public Task<IEnumerable<WalletAddress_OLD>> GetTezosTokenAddressesByContractAsync(
            string tokenContract)
        {
            try
            {
        ...
        public Task<IEnumerable<WalletAddress_OLD>> GetUnspentTezosTokenAddressesAsync(
            string currency,
            string tokenContract,
            decimal tokenId)
        {
        ...
        public Task<bool> UpsertTransactionAsync(IBlockchainTransaction_OLD tx)
        {
            try
            {
                lock (_syncRoot)
            ...
            Type transactionType)
        {
            try
            {
                lock (_syncRoot)
                {
                    ...
                    if (document != null)
                    {
                        var tx = (IBlockchainTransaction_OLD)_bsonMapper.ToObject(
                            type: transactionType,
                            doc: document);
            ...
            return Task.FromResult<IBlockchainTransaction_OLD>(null);
        }

        public Task<IEnumerable<IBlockchainTransaction_OLD>> GetTransactionsAsync(
            string currency,
            Type transactionType)
        {
            try
            {
                lock (_syncRoot)
                {
                    ...
                    var transactions = db.GetCollection(TransactionCollectionName)
                        .Find(Query.EQ(CurrencyKey, currency))
                        .Select(d => (IBlockchainTransaction_OLD)_bsonMapper.ToObject(
                            type: transactionType,
                            doc: d))
                        .ToList();

                    return Task.FromResult<IEnumerable<IBlockchainTransaction_OLD>>(transactions);
                }
            }
            catch (Exception e)
            {
            ...
            return Task.FromResult(Enumerable.Empty<IBlockchainTransaction_OLD>());
        }

        public async Task<IEnumerable<IBlockchainTransaction_OLD>> GetUnconfirmedTransactionsAsync(
            string currency,
            Type transactionType)
        {
