...
namespace Atomex.Cryptography.BouncyCastle
{
    public class BcEd25519
    {
        ...
        private static byte[] CalculateS(ReadOnlySpan<byte> r, ReadOnlySpan<byte> k, ReadOnlySpan<byte> s)
        {
            var t = new uint[ScalarUints * 2]; DecodeScalar(r, 0, t);
            var u = new uint[ScalarUints]; DecodeScalar(k, 0, u);
            var v = new uint[ScalarUints]; DecodeScalar(s, 0, v);

            Nat256.MulAddTo(u, v, t);

            var result = new byte[ScalarBytes * 2];
        ...
        private static bool CheckContextVar(ReadOnlySpan<byte> ctx, byte phflag)
        {
        ...
        private static bool CheckScalarVar(ReadOnlySpan<byte> s)
        {
        ...
        private static IIncrementalHash CreateDigest() => HashAlgorithm.Sha512.CreateIncrementalHash();
        private static int GetDigestSize() => 64;

        private static uint Decode24(ReadOnlySpan<byte> bs, int off)
        {
            uint n = bs[off];
            n |= (uint)bs[++off] << 8;
            n |= (uint)bs[++off] << 16;
        ...
        private static uint Decode32(ReadOnlySpan<byte> bs, int off)
        {
            uint n = bs[off];
            n |= (uint)bs[++off] << 8;
            n |= (uint)bs[++off] << 16;
        ...
        private static void Decode32(ReadOnlySpan<byte> bs, int bsOff, uint[] n, int nOff, int nLen)
        {
            for (int i = 0; i < nLen; ++i)
                n[nOff + i] = Decode32(bs, bsOff + i * 4);
        }

        private static bool DecodePointVar(ReadOnlySpan<byte> p, int pOff, bool negate, PointExt r)
        {
            var py = new byte[PointBytes];

            p.CopyTo(new Span<byte>(py));
        ...
        private static void DecodeScalar(ReadOnlySpan<byte> k, int kOff, uint[] n)
        {
            Decode32(k, kOff, n, 0, ScalarUints);
        }

        private static void Dom2(IIncrementalHash d, byte phflag, ReadOnlySpan<byte> ctx)
        {
            if (ctx != null && ctx.Length > 0)
            {
                d.Update(Dom2Prefix);
                d.Update(new ReadOnlySpan<byte>(new byte[] { phflag, (byte)ctx.Length }));
                d.Update(ctx);
            }
        }

        private static void Encode24(uint n, Span<byte> bs, int off)
        {
            bs[off] = (byte)n;
            bs[++off] = (byte)(n >> 8);
            bs[++off] = (byte)(n >> 16);
        ...
        private static void Encode56(ulong n, Span<byte> bs, int off)
        {
            Encode32((uint)n, bs, off);
            Encode24((uint)(n >> 32), bs, off + 4);
        }

        private static int EncodePoint(PointAccum p, Span<byte> r, int rOff)
        {
            ...
            var rBytes = new byte[r.Length];

            X25519Field.Encode(y, rBytes, rOff);
            rBytes[rOff + PointBytes - 1] |= (byte)((x[0] & 1) << 7);

            new Span<byte>(rBytes).CopyTo(r);
        ...
        {
            using var d = CreateDigest();

            var h = stackalloc byte[GetDigestSize()];
            var hSpan = new Span<byte>(h, GetDigestSize());

            d.Update(sk);
            d.Finalize(hSpan);
  
            var s = stackalloc byte[ScalarBytes];
            var sSpan = new Span<byte>(s, ScalarBytes);
            hSpan.Slice(0, ScalarBytes).CopyTo(sSpan);

            PruneScalar(sSpan);

            ScalarMultBaseEncoded(sSpan, pk);
        }
        ...
        private unsafe static void ImplSign(
            IIncrementalHash d,
            Span<byte> h,
            ReadOnlySpan<byte> s,
            ...
            Span<byte> sig)
        {
            d.Initialize();
            Dom2(d, phflag, ctx);
            d.Update(h.Slice(ScalarBytes, ScalarBytes));
            d.Update(m);
            d.Finalize(h);

            var r = ReduceScalar(h);
            var R = stackalloc byte[PointBytes];
            var RSpan = new Span<byte>(R, PointBytes);
            ScalarMultBaseEncoded(r, RSpan);

            //d = CreateDigest(); // todo: check!
            d.Initialize();
            Dom2(d, phflag, ctx);
            d.Update(RSpan);
            d.Update(pk.Slice(0, PointBytes));
            d.Update(m);
            d.Finalize(h);

            var k = ReduceScalar(h);
            var S = CalculateS(r, k, s);
            var SSpan = new Span<byte>(S);

            RSpan.CopyTo(sig);
            SSpan.Slice(0, ScalarBytes).CopyTo(sig.Slice(PointBytes, ScalarBytes));
        }
            ...
            Span<byte> sig)
        {
            ...
            d.Update(sk.Slice(0, SecretKeySize));
            d.Finalize(hSpan);
            ...
            var pk = stackalloc byte[PointBytes];
            var pkSpan = new Span<byte>(pk, PointBytes);
            ScalarMultBaseEncoded(sSpan, pkSpan);

            ImplSign(d, hSpan, sSpan, pkSpan, ctx, phflag, m, sig);
        }
            ...
            Span<byte> sig)
        {
            ...
            ImplSign(d, hSpan, sSpan, pk, ctx, phflag, m, sig);
        }

        private unsafe static bool ImplVerify(
            ...
            ReadOnlySpan<byte> m)
        {
            ...
            var R = sig.Slice(0, PointBytes); // from sifOff to sigOff + PointBytes
            var S = sig.Slice(PointBytes, PointBytes);  // from sifOff + PointBytes to sigOff + SignatureSize
            ...
            var pA = new PointExt();
            if (!DecodePointVar(pk, 0, true, pA))
                return false;
            ...
            Dom2(d, phflag, ctx);
            d.Update(R.Slice(0, PointBytes));
            d.Update(pk.Slice(0, PointBytes));
            d.Update(m);
            d.Finalize(hSpan);

            var k = ReduceScalar(hSpan);

            var nS = new uint[ScalarUints];
            DecodeScalar(S, 0, nS);

            var nA = new uint[ScalarUints];
            DecodeScalar(k, 0, nA);

            var pR = new PointAccum();
            ScalarMultStrausVar(nS, nA, pA, pR);

            var check = new byte[PointBytes];
            return 0 != EncodePoint(pR, check, 0) && R.SequenceEqual(check);
        }
        ...
        public static void Precompute()
        {
            lock (precompLock)
            {
                ...
                {
                    var ds = new PointExt[PrecompTeeth];

                    var sum = new PointExt();
                    PointSetNeutral(sum);
                    ...
                    var points = new PointExt[PrecompPoints];
                    int k = 0;
                    points[k++] = sum;
        ...
        private static void PruneScalar(Span<byte> r)
        {
            r[0] &= 0xF8;
        ...
        private static byte[] ReduceScalar(ReadOnlySpan<byte> n)
        {
        ...
        private static void ScalarMultBase(ReadOnlySpan<byte> k, PointAccum r)
        {
            ...
            {
                var c1 = Nat.CAdd(ScalarUints, ~(int)n[0] & 1, n, L, n); Debug.Assert(c1 == 0);
                var c2 = Nat.ShiftDownBit(ScalarUints, n, 1U); Debug.Assert(c2 == (1U << 31));

                for (var i = 0; i < ScalarUints; ++i)
                    n[i] = Interleave.Shuffle2(n[i]);
            }

            var p = new PointPrecomp();
        ...
        private static void ScalarMultStrausVar(uint[] nb, uint[] np, PointExt p, PointAccum r)
        {
            ...
            var ws_b = GetWnafVar(nb, WnafWidthBase);
            var ws_p = GetWnafVar(np, width);

            var tp = PointPrecompVar(p, 1 << (width - 2));
            ...
            Span<byte> signature)
        {
            ...
            ImplSign(privateKey, ctx, phflag, data, signature);
        }

        public static byte[] SignWithExtendedKey(
            ReadOnlySpan<byte> privateKey,
            ReadOnlySpan<byte> data)
        {
            var signature = new byte[SignatureSize];

            SignWithExtendedKey(privateKey, data, signature);

            return signature;
        }

        public static void SignWithExtendedKey(
            ReadOnlySpan<byte> extendedPrivateKey,
            ReadOnlySpan<byte> data,
            Span<byte> signature)
        {
            byte[] ctx = null;
            ...
            using var pk = new UnmanagedBytes(PointBytes);
            ScalarMultBaseEncoded(extendedPrivateKey.Slice(0, 32), pk);

            using var h = new UnmanagedBytes(GetDigestSize());
            extendedPrivateKey.Slice(0, GetDigestSize()).CopyTo(h);

            using var d = CreateDigest();

            ImplSign(d, h, extendedPrivateKey, pk, ctx, phflag, data, signature);
        }
            ...
            ReadOnlySpan<byte> signature)
        {
            ...
            return ImplVerify(signature, publicKey, ctx, phflag, data);
        }
        ...
        {
            using var unmanagedPrivateKey = new UnmanagedBytes(SecretKeySize);
            using var unmanagedPublicKey = new UnmanagedBytes(PublicKeySize);

            // copy first 32-bytes from seed to expandedPrivateKey left part ([0-31] bytes)
            seed.GetReadOnlySpan()
                .Slice(0, SecretKeySize)
                .CopyTo(unmanagedPrivateKey);

            // use first 32-bytes from seed (private key) to generate public key
            GeneratePublicKey(unmanagedPrivateKey, unmanagedPublicKey);

            privateKey = new SecureBytes(unmanagedPrivateKey);
            publicKey = new SecureBytes(unmanagedPublicKey);
        }
        ...
        public static void GenerateKeyPair(
            ...
            out SecureBytes publicKey)
        {
            using var unmanagedBytes = seed.ToUnmanagedBytes();

            GenerateKeyPair(unmanagedBytes, out privateKey, out publicKey);
        }
        ...
        public static void GeneratePublicKeyFromExtended(
            ...
            out SecureBytes publicKey)
        {
            using var unmanagedExtentedPrivateKey = extendedPrivateKey.ToUnmanagedBytes();
            using var unmanagedPublicKey = new UnmanagedBytes(PointBytes);

            // calculate public key from entended 64-byte key
            ScalarMultBaseEncoded(unmanagedExtentedPrivateKey, unmanagedPublicKey);
